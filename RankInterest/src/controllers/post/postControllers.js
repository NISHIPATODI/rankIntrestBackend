  ...
  const InsertLike = async (req, res) => {
   ...
   console.log("userId   "+userId);
    ...
    let [Likerror, like_inserted] = await to(
      Like.query()
        .insert({ userId:userId, postId:postId,isLike:isLike })
        .returning("*")
    );
...
console.log("comment's detail ", comment_inserted.commentId+" comment details"+ comment_inserted);
   ...
   var final= result1.concat(result2);
    ...
    const AddCategory = async (req, res) => {
      ...
      let [errorCat, cat_inserted] = await to(
        Category.query()
          .insert({ userId:userId, category:category, catImage:image})
          .returning("*")
      );
